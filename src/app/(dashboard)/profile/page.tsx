  ...
  const sessionColumns: ColumnDef<UserSession>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">{parseUserAgent(row.original.userAgent)}</span>
        </div>
      ),
    },
    {
    ...
    <>
      ...
      <div className="max-w-xl flex flex-col gap-5">
        ...
        <section className="bg-card border border-border rounded-2xl p-6">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-5">{t.profile.account}</h2>

          <div className="flex items-center gap-4 mb-5">
            <div className="w-14 h-14 rounded-full bg-brand/20 border border-brand/20 flex items-center justify-center text-lg font-semibold text-brand-text shrink-0">
              {initials}
            </div>
            <div className="min-w-0">
              <p className="text-base font-semibold text-foreground truncate">
                {user?.name ?? <span className="text-muted-foreground italic text-sm">{t.profile.noName}</span>}
              </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="flex flex-col gap-1 px-4 py-3 rounded-xl bg-overlay-subtle border border-border">
              <span className="text-xs text-muted-foreground uppercase tracking-wider font-medium">{t.profile.emailLabel}</span>
              <span className="text-sm text-foreground font-mono">{user?.email}</span>
            </div>
            <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-overlay-subtle border border-border">
              <div>
                <span className="text-xs text-muted-foreground uppercase tracking-wider font-medium block mb-0.5">
        ...
        {/* Language */}
        <section className="bg-card border border-border rounded-2xl p-6">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-5">{t.profile.language}</h2>
          <div className="flex items-center gap-3">
            ...
            {isSavingLocale && <Loader2 className="w-4 h-4 motion-safe:animate-spin text-muted-foreground" />}
          </div>
        ...
        {/* Change password */}
        <section className="bg-card border border-border rounded-2xl p-6">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-5">{t.profile.changePassword}</h2>
          ...
          <form onSubmit={handleSubmit(handleChangePassword)} className="flex flex-col gap-4">
            <FormField label={t.profile.currentPassword} name="currentPassword" error={errors.currentPassword?.message} required>
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-0 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center text-muted-foreground hover:text-muted-foreground transition-colors cursor-pointer"
                  tabIndex={-1}
                >
            ...
            <div className="flex justify-end pt-1">
              <button
                ...
                className="inline-flex items-center gap-2 bg-brand hover:bg-brand-hover text-white px-5 py-2 rounded-xl text-sm font-medium transition-colors disabled:opacity-50"
              >
        ...
        <section className="bg-card border border-border rounded-2xl p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">{t.profile.sessions}</h2>
            {sessions.length > 0 && (
              <button
                onClick={() => setLogoutAllOpen(true)}
                className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-red-400 transition-colors cursor-pointer"
              >
